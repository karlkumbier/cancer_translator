...
p <- rbind(xplot1, xplot2) %>%
  ...
  geom_text(data=xtext, x=3, y=1.75, size=5, aes(label=Score)) +
...
col.order <- order(
  xpa[bioactive.order[1],],
  xpa[bioactive.order[2],],
  xpa[bioactive.order[3],],
  xpa[bioactive.order[4],],
  xpa[bioactive.order[5],],
  xpa[bioactive.order[6],]
)
