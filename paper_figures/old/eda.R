...
BiocManager::install("ChemmineOB", configure.args='--with-openbabel-include=/Users/kkumbier/github/ob-build/include --with-openbabel-lib=/Users/kkumbier/github/ob-build/lib') 
...
#' is associated with features indicating (1) name (2) identifier (3) site/histology
#' (4) growth conditions.
#' 
#' **QUESTION 1:** Do we want to restrict our analysis to any specific subset
#' of these cell lines based on e.g. growth conditions.
#' 
#' **QUESTION 2:** Do we want to incorporate any of these features into cell 
...
#' compounds in the dataset target multiple proteins (see figure below). In total,
#' the CTRP compounds target 322 unique compounds (+1 NA).
#' 
#'  **Question 3:** How should compound target be incorporated into optimization
#'  criteria? E.g. Optimizing over targets/proteins. 
#'  
#'  **Question 4:** Should compound target be used for filtering/cleaning/validating
#'  bioactivity data?
...
superheat(xtarget, title='Compound targets')
...
group_by(cpd.table, TargetsClean) %>%
  ...
  theme_bw() +
  coord_flip() +
  ggtitle('# Compounds by target class')
...
#' ## Bioactivity
#' The bioactivity data measures area under the curve (AUC) — derived from ??? 
#' evaluated at multiple concentrations — for cell line/compound pairs. We
#' convert these data into a <n cell lines> x <n compounds> matrix.
#' 
...
data.frame(AUC=colSums(x <= 10)) %>%
  group_by(AUC) %>%
  summarize(Count=n()) %>%
  ggplot(aes(x=AUC, y=Count)) +
  geom_point() +
  theme_bw() +
  xlab('# Cell lines with AUC < 10')

#' **Question 5:** we are interested in cell lines with high coverage across the 
#' compound set. In particular, using a small number of cell lines to cover many 
#' compounds. Compounds covered by a small # of cell lines are unlikely to be
#' detected in this way. How do we want to handle this?
#' 
...
data.frame(Target=cpd.table$TargetsClean, AUC=xmean) %>%
  group_by(Target) %>%
  summarize(AUC=min(AUC), .groups='drop') %>%
  ggplot(aes(x=reorder(Target, AUC), y=AUC)) +
  geom_bar(stat='identity') +
  coord_flip() +
  theme_bw() +
  ggtitle('Minimum AUC by target')

# Average bioactivity matrix within compound target
targets <- unique(cpd.table$TargetsClean)
xgroup <- sapply(targets, function(tgt) {
  return(rowMeans(xplot[,cpd.table$TargetsClean == tgt]))
})

superheat(xgroup,
          pretty.order.rows=TRUE,
          pretty.order.cols=TRUE,
          membership.cols=targets,
          heat.pal=cols,
          heat.pal.values=c(0, 0.5, 0.6, 0.7, 0.8, 0.9, 1),
          bottom.label.text.angle=90,
          bottom.label.size=1,
...
y <- as.numeric(as.factor(cpd.table$TargetsClean[id.select]))
fit <- glmnet(x=t(x)[id.select,], y=y, family='multinomial')

# Model coefficient summaries
betas <- lapply(coef(fit), abs)
betas <- Reduce('+', betas)
cell.counts <- colSums(betas != 0) - 1

# Model prediction summaries
ypred <- predict(fit, t(x)[id.select,], type='response')

# Summarize model predictions using different #s of cell lines
target.levels <- levels(as.factor(cpd.table$TargetsClean[id.select]))
n.cells <- c(5, 10, 15, 20, 25, 50)
accuracy <- c()
ncells <- c()
for (i in n.cells) {
  
  id.select <- max(which(cell.counts <= i))
  ncell <- cell.counts[id.select]
  xplot <- apply(ypred[,,id.select], MAR=2, function(z) z / max(z))
  ypred.top <- apply(xplot, MAR=1, which.max)
  accuracy <- c(accuracy, mean(ypred.top == y))
  ncells <- c(ncells, ncell)
  
  superheat(xplot, 
            membership.rows=target.levels[y],
            membership.cols=target.levels,
            pretty.order.rows=TRUE,
            pretty.order.cols=TRUE,
            title=str_c('Predictions: # cell lines = ', ncell),
            heat.pal=cols,
