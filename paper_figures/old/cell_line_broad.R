...
# Compoute inverse covariance matrix
...
ggplot(xdist, aes(x=K, y=Score)) +
  geom_boxplot(col=col.pal[4], fill=col.pal[4], alpha=0.7) +
  geom_point(data=xopt, shape=8, col='#DA1613') +
...
ggplot(xdist, aes(x=K, y=Score)) +
  ...
  geom_point(data=xopt, shape=8, col='#DA1613') +
  geom_text(data=xopt, aes(label=CellSet), nudge_y=0.025, size=5) +
  theme(axis.text.x=element_text(angle=90), text=element_text(size=18)) +
  ylab('Proportion bioactive') +
  xlab('# cell lines') +
  ggtitle('Proportion bioactive compounds in select categories by optimal cell set')
